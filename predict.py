...
def predict(x, model, patch_sz=160, n_classes=5):
    ...
    # print(npatches_vertical, npatches_horizontal, np.shape(ext_x), np.shape(x))
    patches_list = []
    for i in range(0, npatches_vertical):
        for j in range(0, npatches_horizontal):
            x0, x1 = i * (patch_sz-20), (i + 1) * (patch_sz-20)+20
            y0, y1 =  j * (patch_sz-20), (j + 1) * (patch_sz-20)+20
            patches_list.append(ext_x[x0:x1, y0:y1, :])
    # model.predict() needs numpy array rather than a list
    patches_array = np.asarray(patches_list)
    # print(np.shape(patches_array), patches_array)
    # predictions:
    patches_predict = model.predict(patches_array, batch_size=4)
    print("shape of patches_predict:",np.shape(patches_predict))
    prediction = np.zeros(shape=(extended_height, extended_width, n_classes), dtype=np.float32)
    count_patches = np.zeros(shape=(extended_height, extended_width, n_classes))
    for k in range(patches_predict.shape[0]):
        i = k // npatches_horizontal
        j = k % npatches_vertical
        x0, x1 = i * (patch_sz-20), (i + 1) * (patch_sz-20)+20
        y0, y1 =  j * (patch_sz-20), (j + 1) * (patch_sz-20)+20
        prediction[x0:x1, y0:y1, :] = np.add(prediction[x0:x1, y0:y1, :], patches_predict[k, :, :, :])
        count_patches[x0:x1, y0:y1, :] = np.add(count_patches[x0:x1, y0:y1, :], np.ones(np.shape(prediction[x0:x1, y0:y1, :])))
    #prediction=np.divide(prediction, count_patches)
    final=np.divide(prediction[:img_height, :img_width, :], count_patches[:img_height, :img_width, :])
    return final
...
if __name__ == '__main__':
    ...
    #print(mymat[0][0][0], mymat[3][12][13])
    map, _ = picture_from_mask(mymat, 0.5)
    #mask = predict(img, model, patch_sz=PATCH_SZ, n_classes=N_CLASSES).transpose([2,0,1])  # make channels first
    #map = picture_from_mask(mask, 0.5)
